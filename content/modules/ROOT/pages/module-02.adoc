...
NOTE: The following section provides additional information on the Service Mesh Architecture. *Feel free to skip the following section to save time* and come back later if needed.
...
NOTE: All assets for this module are in the folder `module-02` in the repo you cloned in the introdcution module. Please change the directory into this folder now.
...
Older versions would require that each of the individual services **Istio**, **Kiali**, and **Jaeger** be exposed through their own routes, and be made accessible through a console login. The newer version is much more streamlined and easier for users to begin working with.
...
Leave the `Jaeger` dashboard open in your browser, as we will come back to it a little bit later and lets explore the next observability tool.
...
image::rh1/200-mod2-travel-agency-pods.png[link=self, window=blank]
...
First we create a `ServiceMeshMember` resource for each of the 3 namespaces of the _Travel Booking_ application, which will include them to the mesh. (click for details of the configuration {modernize_ocp_virt_repo}/blob/main/module-02/servicemeshmember.yaml[servicemeshmember.yaml, window=_blank]). Now execute in the *Terminal* the following:
...
====
Access the _Travel Business_ dashboard https://travel-dashboard-travel-control.{modernize_ocp_virt_apps_domain}[window=_blank].
====
