...
image::summit/201-OSSM-ServiceMesh.png["Red Hat OpenShift Service Mesh overview diagram", link=self,window=blank,width=100%]
...
The following section provides additional information on the Service Mesh Architecture. *Feel free to skip to the Getting Started section to save time* and come back later if needed.

=== OpenShift Service Mesh architecture

image::summit/202-OSSM-Components.png["Service Mesh architecture showing istiod, envoy sidecars, Kiali, Prometheus, Grafana, and gateways", link=self,window=blank,width=100%]
...
image::summit/203-service-mesh-overview.png["Service Mesh Overview page listing projects and application health status", link=self,window=blank,width=100%]

On this screen you will see that there are all of the projects in our shared cluster listed. In this lab we will be working with the projects: *{modernize_ocp_virt_username}-travel-agency*, *{modernize_ocp_virt_username}-travel-control*, and *{modernize_ocp_virt_username}-travel-portal*.
...
Use the *Namespace* dropdown at the top and search for *{modernize_ocp_virt_username}-travel* to see your associate projects.
Select the projects: *{modernize_ocp_virt_username}-travel-agency*, *{modernize_ocp_virt_username}-travel-control*, and *{modernize_ocp_virt_username}-travel-portal*.

image::summit/204-traffic-graph-projects.png["Namespace dropdown with travel-agency, travel-control, and travel-portal projects selected", link=self,window=blank,width=100%]

When the projects are selected there will be a brief pause, and the console will attempt to load the traffic graph.

image::summit/205-traffic-graph-empty.png["Empty traffic graph with no data because applications are not yet in the mesh", link=self,window=blank,width=100%]
...
image::summit/200-mod2-travel-agency-pods.png["Pods list in the travel-agency project showing VM-associated pods", link=self, window=blank,width=100%]

Right now you may notice that the pods associated with each VM have *2 containers*.

image::summit/206-travel-agency-two-containers.png["Pod details showing two containers: compute and guest-console-log", link=self, window=blank,width=100%]
...
image::summit/207-pod-container-list-a.png["Pod container list showing the compute container", link=self, window=blank,width=100%]
image::summit/207-pod-container-list-b.png["Pod container list showing the guest-console-log container", link=self, window=blank,width=100%]
...
image::summit/208a-terminal-post-scripts.png["Terminal output after running the sidecar injection scripts", link=self, window=blank,width=100%]
image::summit/208b-terminal-post-scripts.png["Terminal output after running the sidecar injection scripts", link=self, window=blank,width=100%]
...
image::summit/209-travel-agency-three-containers.png["Pod list now showing three containers per pod after sidecar injection", link=self, window=blank,width=100%]

image::summit/210-cars-vm-three-containers.png["cars-vm pod details showing compute, guest-console-log, and istio-proxy containers", link=self, window=blank,width=100%]
...
image::summit/211-traffic-graph-data.png["Traffic graph showing network flows between VMs and containers in the mesh", link=self, window=blank,width=100%]
...
image::summit/212-legend-button.png["Traffic graph with the Legend button highlighted at the bottom", link=self, window=blank,width=100%]
...
We are going to simulate a booking request by sending a request for a travel quote from the `travels` portal in the *{modernize_ocp_virt_username}-travel-portal* namespace to the `travels-vm` VM in the *{modernize_ocp_virt_username}-travel-agency* namespace.
...
image::summit/213-istio-config.png["Service Mesh Istio Config page showing the gateway configuration", link=self, window=blank,width=100%]
...
image::summit/214-istio-gateway-terminal.png["Terminal showing successful HTTPRoute creation for the control-vm service", link=self, window=blank,width=100%]

If you refresh the page on your OpenShift console you should see that there is now an HTTPRoute created in the Istio Config pane.

image::summit/214a-istio-httproute.png["New HTTPRoute in Istio Config", link=self, window=blank,width=100%]
...
image::summit/215-travel-control-dashboard.png["Travel Control dashboard accessible through the Istio gateway", link=self, window=blank,width=100%]
...
Ensure that you are in the *{modernize_ocp_virt_username}-travel-agency* project in your terminal on the right, and then execute the script below which delivers all the above configurations:
...
With the script completed successfully, click on `Virtualization -> VirtualMachines` and view the new `cars-vm-v2` running in the *{modernize_ocp_virt_username}-travel-agency* project.

image::summit/216-cars-vm-v2.png["VirtualMachines list showing the new cars-vm-v2 running alongside existing VMs", link=self, window=blank,width=100%]

Now click on `Service Mesh -> Istio Config`. This will now list the new traffic management configurations under the `{modernize_ocp_virt_username}-travel-agency` project.

image::summit/217-istio-config-traffic.png["Istio Config page showing new DestinationRule and VirtualService for traffic management", link=self, window=blank,width=100%]
...
image::summit/218-traffic-graph-v1-v2.png["Traffic graph showing cars-vm directing traffic to both v1 and v2 versions", link=self, window=blank,width=100%]
...
image::summit/219-traffic-distribution.png["Traffic distribution view showing 90/10 split between v1 and v2", link=self, window=blank,width=100%]
...
image::summit/220-traffic-distribution-80.png["Traffic distribution view showing 20/80 split with 80 percent flowing to v2", link=self, window=blank,width=100%]
...
image::summit/221-traffic-graph-fail.png["Traffic graph showing failures between services after deny-all policy applied", link=self, window=blank,width=100%]
...
image::summit/222-access-denied.png["Browser showing RBAC access denied message when accessing the Travel Booking Dashboard", link=self, window=blank,width=100%]
...
* From services in the *{modernize_ocp_virt_username}-travel-portal* *->* to services in *{modernize_ocp_virt_username}-travel-agency* and *{modernize_ocp_virt_username}-travel-control*.
...
Returning to `Service Mesh -> Traffic Graph` you should see that connectivity begins to be restored between the *{modernize_ocp_virt_username}-travel-portal* and *{modernize_ocp_virt_username}-travel-agency* projects. You can refresh the graph to see the changes take place.

image::summit/223a-traffic-restored.png["Traffic graph showing restored connectivity after applying selective authorization policies", link=self, window=blank,width=100%]

image::summit/223b-traffic-restored.png["Traffic graph showing restored connectivity after applying selective authorization policies", link=self, window=blank,width=100%]

image::summit/223c-traffic-restored.png["Traffic graph showing restored connectivity after applying selective authorization policies", link=self, window=blank,width=100%]
...
image::summit/224-dashboard-restored.png["Travel Booking Dashboard loaded successfully after authorization policies restored access", link=self, window=blank,width=100%]
...
[source,shell,subs="attributes",role=execute]
----
oc -n {modernize_ocp_virt_username}-travel-control run toolbox --rm -it --image=registry.redhat.io/rhel9/toolbox --restart=Never -overrides='
{
  "spec": {
    "containers": [
      {
        ...
        "command": ["/bin/bash", "-c"],
        "args": ["curl -si travels-vm.{modernize_ocp_virt_username}-travel-agency.svc.cluster.local:8000/travels/London"],
        "resources": {
...
IMPORTANT: Prior to moving on to the next section, to conserve resources and ensure a smooth lab experience, make sure that any of the VMs currently running in the *{modernize_ocp_virt_username}-travel-agency* and *{modernize_ocp_virt_username}-travel-control* projects are shut down. We will not be making use of them again.

image::summit/225-all-vms-stopped.png["All VMs are stopped", link=self, window=blank,width=100%]
