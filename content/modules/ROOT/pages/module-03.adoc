// This module was moved to module-02, this is old and should be deleted. Leaving here for review and backup.

// Circuit breaker was not moved to module-02

= Module 3 - Enhancing Security with OpenShift Service Mesh
...
====
Open the URL to the Service Mesh _Ingress Gateway_ at http://istio-ingressgateway-istio-system.{modernize_ocp_virt_apps_domain}/[window=_blank]
...
Execute the script {modernize_ocp_virt_repo}/blob/main/module-03/expose-control-vm.sh[expose-control-vm.sh, window=_blank] in the *Terminal*  to deploy these configurations:

[,sh,subs="attributes",role=execute]
----
./expose-control-vm.sh {modernize_ocp_virt_apps_domain}
----

====
Refresh the URL of the _Ingress Gateway_ at http://istio-ingressgateway-istio-system.{modernize_ocp_virt_apps_domain}[window=_blank] and you should be served with the familiar Travel Booking Dashboard.
...
. In order to achieve this, you are going to deploy a new VM with the name `cars-vm-v2-a` and a label `version=v2` (see details {modernize_ocp_virt_repo}/blob/main/module-03/cars-vm-v2-a.yaml#L38[here,window=_blank]).

. Then you need to configure a `DestinationRule` to be able to direct the traffic to both versions of the `cars-vm` service. (see details {modernize_ocp_virt_repo}/blob/main/module-03/multipleversions-for-car-vm-in-the-mesh.sh#L30-L36[here,window=_blank]).
+
...
* First, you deploy an additional VM with the name {modernize_ocp_virt_repo}/blob/main/module-03/cars-vm-v2-b.yaml[`cars-vm-v2-b`].
...
Apply the following resource to deploy the new {modernize_ocp_virt_repo}/blob/main/module-03/cars-vm-v2-b.yaml[`VM`,window=_blank].
...
Now apply the {modernize_ocp_virt_repo}/blob/main/module-03/circuit-breaker.sh#L54-L66[circuit-breaker.sh, window=_blank] script:
...
NOTE: You can confirm the effect by accessing the http://istio-ingressgateway-istio-system.{modernize_ocp_virt_apps_domain}/[Travel Booking Dashboard, window=_blank] which now returns `RBAC: access denied`.
...
====
After a short period you should gain access to the http://istio-ingressgateway-istio-system.{modernize_ocp_virt_apps_domain}/[Travel Booking Dashboard, window=_blank] and the `Kiali` dashboard will show a restored network of communications between the services. 
====
