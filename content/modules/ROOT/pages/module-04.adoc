...
----
cd $HOME/modernize-ocp-virt-workspace/lab-5
----
...
====
Connect the `{modernize_ocp_virt_repo}` repo to Argo CD by setting repository url to the github repo url, leave revision as `HEAD`, and set the path to `lab-5/travel-control`:
