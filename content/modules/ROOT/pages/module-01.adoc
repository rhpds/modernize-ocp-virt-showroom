...
[source,shell,subs=attributes,role=execute]
----
Please help me create a virtual machine in the user1-vms project for a database server with 2 CPUs and 4 GB of RAM from an instancetype that boots from the centos-stream-9 volume in the openshfit-virtualization-os-images project. Ensure that the VM automatically installs mariadb and configures it to run on startup.
----

image::rh1/28-create-vm-prompt.png[link="self", window=blank, width="100%"]

OpenShift Lightspeed will respond with step-by-step instructions on how to create our VM, and provides the associated commands that we can copy/paste directly into our terminal. Lets step through this process together.

Step 1: Create an InstanceType using the `virtctl` command to help us create the VM with the correct physical resources.

image::rh1/29-step-one.png[link="self", window=blank, width="100%"]





Step one is to create an instancetype with our specified resources using the `virtctl` CLI command. Lets perform this now using our terminal on the right.
