...
[,sh,subs="attributes",role=execute]
----
cd $HOME/modernize-ocp-virt-workspace/lab-1
----
////
...
## Task 2: Explore Kubernetes Services for Virtual Machines
...
All VMs already have services configured, so they can connect to each other. 
...
[,sh,subs="attributes",role=execute]
----
curl -v http://travels-vm.{modernize_ocp_virt_username}-travel-agency.svc.cluster.local:8000/travels/London
----
...
Now, if we want to make the `travels` API service accessable from outside of the cluster, we need an OpenShift Route to expose the service:
...
Let us test the Route.

[,sh,subs="attributes",role=execute]
----
export TRAVELS_ROUTE=$(oc get route travels-vm -o jsonpath='{.spec.host}' -n {modernize_ocp_virt_username}-travel-agency)
----
...
[,sh,subs="attributes",role=execute]
----
oc annotate route travels-vm haproxy.router.openshift.io/disable_cookies='true' -n {modernize_ocp_virt_username}-travel-agency
----
...
[,sh,subs="attributes",role=execute]
----
oc delete networkpolicy noegress -n {modernize_ocp_virt_username}-travel-agency
----

// TODO Moved Business Dashboard from module 2 as it still make sense to keep in the lab.
// TODO: Refine the section and reorder if needed.

## Task 6: Checkout the Business Dashboard
...
[,sh,subs="attributes",role=execute]
----
echo "$(oc get route travel-control -o jsonpath='{.spec.host}' -n {modernize_ocp_virt_username}-travel-control)"
----
