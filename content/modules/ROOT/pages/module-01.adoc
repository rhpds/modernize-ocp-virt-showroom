...
[source,shell,subs=attributes,role=execute]
----
Create a YAML manifest that creates a VM in the user1-vms project that has CentOS Stream 9 as it's operating system, to be used as a database-server. Configure it with a username and password through cloudinit, and automatically install mariadb and configure it to run on startup.
----
      ...
      - name: cloudinitdisk
        cloudInitNoCloud:
          userData: |
            ...
            users:
              - name: dbadmin
                passwd: "$6$kzJOz9FOlJUyAVz1$rqLhnsj6by5qeEo0tqFazm9zZ2WbGOU4fUv8Ewo7sg7hkL1qg2Dv.rU7OIyNxxJp1VHAPYUoKrkEL3UV13.MF/"
                groups: sudo
                shell: /bin/bash
            packages:
              - mariadb-server
...
NOTE: OpenShift Lightspeed is not currently able to generate the hashed password needed for cloud-init. In the generated yaml you will see it creates a password `dbpassword` but cloud-init expects a SHA-512 hash. We have provided that hashed password for you in the YAML sample above.
...
Click on the `Console` tab, and use the credentials to login, and verify that mariadb is installed, and running.
