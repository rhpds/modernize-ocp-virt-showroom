...
A new window will launch, name your project {modernize_ocp_virt_username}-vms, leave the other fields blank, and click on the `Create` button.
...
Use the following command to set the project to the {modernize_ocp_virt_username}-vms project:

[source,shell,subs=attributes,role=execute]
----
oc project {modernize_ocp_virt_username}-vms
----
...
IMPORTANT: Prior to moving on to the next section, to conserve resources and ensure a smooth lab experience, make sure that any of the VMs created in the *{modernize_ocp_virt_username}-vms* project as a part of this module are shut down, and/or deleted. We will not be making use of them again.
